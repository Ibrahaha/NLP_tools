# NLP_tools
A container that includes state-of-the-art tools in NLP (Natural Language Processing) for Named Entity Recognition (SpaCy, Stanfordcorenlp, nltk ,Allennlp) and Coreference Resolution (neuralcoref). You can use NER and Coreference Resolution models both as servers or from containers' bash.
...
Be sure that ports 8080 and 8085 are free on your computer (if not change then you need change them in [docker-compose.yml](https://github.com/Ibrahaha/NLP_tools/blob/master/docker-compose.yml).
